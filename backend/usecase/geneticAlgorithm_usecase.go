...
func NewGeneticAlgorithm(data domain.DataSchedule, clientChan chan string, db gorm.DB, UniversityID uint, SemesterID uint) *GeneticAlgorithm {
...
func (ga *GeneticAlgorithm) SelectTimeDetails(subject int) []int {
	...
	selectedDay := days[0]
	startingTime := ga.Settings.StartingTime
	endingTime := ga.Settings.EndingTime
...
func (ga *GeneticAlgorithm) RunGeneration() {
	...
	for ga.Running {
		...
		if ga.HighestFitness >= ga.Settings.MaximumFitness || ga.Settings.MaximumGenerations < generation {
			...
			if err := ga.SaveTopChromosomeDataToDB(topChromosomeData); err != nil {
...
func (ga *GeneticAlgorithm) SaveTopChromosomeDataToDB(topChromosomeData []domain.ChromosomeData) error {
